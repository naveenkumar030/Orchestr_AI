...
export default function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-full w-72 bg-white backdrop-blur-2xl z-50 flex flex-col justify-between border-r border-[#E5DED6] shadow-sidebar transition-transform duration-300 ease-in-out ${
          isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        <div className="flex flex-col flex-1 overflow-y-auto">
          {/* Logo Bar & Mobile Close Button */}
          <div className="px-space-base py-space-lg flex items-start justify-between border-b border-[#E5DED6]/50 lg:border-b-0">
            ...
            <button
              onClick={onClose}
              className="lg:hidden p-1.5 rounded-lg text-[#6B625B] hover:text-[#2D2926] hover:bg-[#F2EDE6] transition-colors"
          ...
          {/* Navigation */}
          <nav className="flex flex-col gap-space-2xs px-space-sm mt-space-sm flex-1">
            {navItems.map((item) => {
              const isActive =
                item.path === '/'
                  ? pathname === '/'
        ...
        <div className="p-space-base m-space-base rounded-xl bg-[#FBF9F5] border border-[#E5DED6] shadow-sm shrink-0">
